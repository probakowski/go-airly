...
func (api Api) Installation(id int) (Installation, error) {
	var i Installation
	err := api.get(fmt.Sprintf("installations/%d", id), &i)
	return i, err
}

func (api Api) NearestInstallations(loc Location, options ...NearestInstallationsOption) ([]Installation, error) {
	var i []Installation
	config := nearestInstallationsConfig{3.0, 1}
	for _, option := range options {
		option(&config)
	}
	err := api.get(fmt.Sprintf("installations/nearest?lat=%f&lng=%f&maxDistanceKM=%f&maxResults=%d",
		loc.Latitude, loc.Longitude, config.maxDistance, config.maxResults), &i)
	return i, err
}

func (api Api) NearestMeasurements(loc Location, options ...NearestInstallationsOption) (Measurements, error) {
	var m Measurements
	config := nearestInstallationsConfig{3.0, 1}
	for _, option := range options {
		option(&config)
	}
	err := api.get(fmt.Sprintf("measurements/nearest?lat=%f&lng=%f&maxDistanceKM=%f",
		loc.Latitude, loc.Longitude, config.maxDistance), &m)
	return m, err
}

func (api Api) PointMeasurements(loc Location) (Measurements, error) {
	var m Measurements
	err := api.get(fmt.Sprintf("measurements/point?lat=%f&lng=%f", loc.Latitude, loc.Longitude), &m)
	return m, err
}

func (api Api) InstallationMeasurements(installationId int) (Measurements, error) {
	var m Measurements
	err := api.get(fmt.Sprintf("measurements/installation?installationId=%d", installationId), &m)
	return m, err
}
...
func (api Api) IndexTypes() ([]IndexType, error) {
	var i []IndexType
	err := api.get("meta/measurements", &i)
	return i, err
}

func (api Api) MeasurementTypes() ([]MeasurementType, error) {
	var m []MeasurementType
	err := api.get("meta/measurements", &m)
	return m, err
}
